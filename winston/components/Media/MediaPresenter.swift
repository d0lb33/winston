...
struct MediaPresenter: View, Equatable {
  ...
  var body: some View {
    let _ = ScrollPerfDiagnostics.bump("mediaPresenterBody")
    switch media {
    case .imgs(let imgsExtracted):
      let _ = ScrollPerfDiagnostics.bump("mediaPresenter.imgs")
      if !showURLInstead {
    ...
    case .streamable(_):
      let _ = ScrollPerfDiagnostics.bump("mediaPresenter.streamable")
      if !showURLInstead {
        ProgressView()
    ...
    case .yt(let ytMediaExtracted):
      let _ = ScrollPerfDiagnostics.bump("mediaPresenter.yt")
      if !showURLInstead {
    ...
    case .post(let postExtractedEntity):
      let _ = ScrollPerfDiagnostics.bump("mediaPresenter.post")
    ...
    case .comment(let commentExtractedEntity):
      let _ = ScrollPerfDiagnostics.bump("mediaPresenter.comment")
    ...
    case .subreddit(let subExtractedEntity):
      let _ = ScrollPerfDiagnostics.bump("mediaPresenter.subreddit")
    ...
    case .user(let userExtractedEntity):
      let _ = ScrollPerfDiagnostics.bump("mediaPresenter.user")
